<?php
	...
	if ( ! defined( 'ABSPATH' ) || ! defined('allegra_upload_cf7') ) {
		exit;
	}
	...
	*/
	...
	add_action( 'wpcf7_init', 'allegra_cf7_upload_add_form_tag_file' );
	add_action( 'wpcf7_enqueue_scripts', 'allegra_cf7_scripts' );

	// Hook language init
	add_action('plugins_loaded','allegra_load_plugin_textdomain');

	// Ajax Upload
	add_action( 'wp_ajax_allegra_codedropz_upload', 'allegra_upload_cf7_upload' );
	add_action( 'wp_ajax_nopriv_allegra_codedropz_upload', 'allegra_upload_cf7_upload' );

	// Hook mail cf7
	add_action('wpcf7_before_send_mail','allegra_cf7_before_send_mail', 30, 1);
	add_action('wpcf7_mail_components','allegra_cf7_mail_components', 50, 2);

	// Add Submenu - Settings
	add_action('admin_menu', 'allegra_admin_settings');

	// Load plugin text-domain
	function allegra_load_plugin_textdomain() {
		load_plugin_textdomain( 'allegra-upload-cf7', false, dirname( dirname( plugin_basename( __FILE__ ) ) ) . '/languages' );
	}

	// Hooks for admin settings
	function allegra_admin_settings() {
		add_submenu_page( 'wpcf7', 'Drag & Drop Uploader - Settings', 'Drag & Drop Upload', 'manage_options', 'drag-n-drop-upload','allegra_upload_admin_settings');
		add_action('admin_init','allegra_upload_register_settings');
	}

	// Default Error Message
	function allegra_cf7_error_msg( $error_key ) {

		// Array of default error message
		$errors = array(
			'server_limit'		=>	__('The uploaded file exceeds the maximum upload size of your server.','allegra-upload-cf7'),
			'failed_upload'		=>	__('Uploading a file fails for any reason','allegra-upload-cf7'),
			'large_file'		=>	__('Uploaded file is too large','allegra-upload-cf7'),
			'invalid_type'		=>	__('Uploaded file is not allowed for file type','allegra-upload-cf7'),
		);
	...
	function allegra_cf7_before_send_mail( $wpcf7 ){
		global $_mail;
		...
		if( $submission ) {
			...
			if( get_option('uploads_use_yearmonth_folders') ) {
				$simple_path = $upload_dir['baseurl'] . '/wp_allegracf7_uploads' . dirname( $upload_dir['subdir'] );
			}
	...
	function allegra_cf7_mail_components( $components, $form ) {
		global $_mail;
	...
	// Load js and css
	function allegra_cf7_scripts() {

		// Get plugin version
		$version = allegra_upload_cf7_version;
		...
		wp_enqueue_script( 'allegra-upload-cf7', plugins_url ('/assets/js/allegra-upload-cf7.js', dirname(__FILE__) ), array('jquery','codedropz-uploader','contact-form-7'), $version, true );

		//  registered script with data for a JavaScript variable.
		wp_localize_script( 'allegra-upload-cf7', 'allegra_cf7_uploader',
			array(
				'ajax_url' 				=> admin_url( 'admin-ajax.php' ),
				'drag_n_drop_upload' 	=> array(
					'text'				=>	( get_option('drag_n_drop_text') ? get_option('drag_n_drop_text') : __('Drag & Drop Files Here','allegra-upload-cf7') ),
					'or_separator'		=>	( get_option('drag_n_drop_separator') ? get_option('drag_n_drop_separator') : __('or','allegra-upload-cf7') ),
					'browse'			=>	( get_option('drag_n_drop_browse_text') ? get_option('drag_n_drop_browse_text') : __('Browse Files','allegra-upload-cf7') ),
					'server_max_error'	=>	( get_option('drag_n_drop_error_server_limit') ? get_option('drag_n_drop_error_server_limit') : allegra_cf7_error_msg('server_limit') ),
				)
			)
		);

		// enque style
		wp_enqueue_style( 'allegra-upload-cf7', plugins_url ('/assets/css/allegra-upload-cf7.css', dirname(__FILE__) ), '', $version );
	}

	// Generate tag
	function allegra_cf7_upload_add_form_tag_file() {
		wpcf7_add_form_tag(	array( 'mfile ', 'mfile*'), 'allegra_cf7_upload_form_tag_handler', array( 'name-attr' => true ) );
	}

	// Form tag handler from the tag - callback
	function allegra_cf7_upload_form_tag_handler( $tag ) {
	...
	add_filter( 'wpcf7_form_enctype', 'allegra_upload_cf7_form_enctype_filter' );

	function allegra_upload_cf7_form_enctype_filter( $enctype ) {
	...
	add_filter( 'wpcf7_validate_mfile', 'allegra_upload_cf7_validation_filter', 10, 2 );
	add_filter( 'wpcf7_validate_mfile*', 'allegra_upload_cf7_validation_filter', 10, 2 );

	function allegra_upload_cf7_validation_filter( $result, $tag ) {
		$name = $tag->name;
	...
	add_action( 'wpcf7_admin_init', 'allegra_upload_cf7_add_tag_generator', 50 );

	function allegra_upload_cf7_add_tag_generator() {
		$tag_generator = WPCF7_TagGenerator::get_instance();
		$tag_generator->add( 'upload-file', __( 'multiple file upload', 'allegra-upload-cf7' ),'allegra_upload_cf7_tag_generator_file' );
	}

	// Display form in admin
	function allegra_upload_cf7_tag_generator_file( $contact_form, $args = '' ) {
	...
	function allegra_upload_cf7_upload() {
		...
		if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
			wp_send_json_error( get_option('drag_n_drop_error_failed_to_upload') ? get_option('drag_n_drop_error_failed_to_upload') : allegra_cf7_error_msg('failed_upload') );
		}

		/* File type validation */
		$file_type_pattern = allegra_upload_cf7_filetypes( $_POST['supported_type'] );

		// validate file type
		if ( ! preg_match( $file_type_pattern, $file['name'] ) ) {
			wp_send_json_error( get_option('drag_n_drop_error_invalid_file') ? get_option('drag_n_drop_error_invalid_file') : allegra_cf7_error_msg('invalid_type') );
		}

		// validate file size limit
		if( $file['size'] > (int)$_POST['size_limit'] ) {
			wp_send_json_error( get_option('drag_n_drop_error_files_too_large') ? get_option('drag_n_drop_error_files_too_large') : allegra_cf7_error_msg('large_file') );
		}
		...
		if( get_option('drag_n_drop_mail_attachment') == 'yes' ) {
			...
			$uploads_dir = apply_filters('allegra_cf7_upload_path', $upload['basedir'] . '/wp_allegracf7_uploads', $upload );

			// Check if upload use year and month folders
			if( get_option('uploads_use_yearmonth_folders') ) {
				$uploads_dir = apply_filters('allegra_cf7_upload_path', $upload['basedir'] . '/wp_allegracf7_uploads'. $upload['subdir'], $upload );
			}
	...
	function allegra_upload_cf7_filetypes( $types ) {
		$file_type_pattern = '';
	...
	// Admin Settings
	function allegra_upload_admin_settings( ) {
		echo '<div class="wrap">';
		...
		?>
				...
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e('Send Attachment as links?','allegra-upload-cf7'); ?></th>
				...
				<h2><?php _e('Uploader Info','allegra-upload-cf7'); ?></h2>

				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e('Drag & Drop Text','allegra-upload-cf7'); ?></th>
					...
					<tr valign="top">
						<th scope="row"><?php _e('Browse Text','allegra-upload-cf7'); ?></th>
				...
				<h2><?php _e('Error Message','allegra-upload-cf7'); ?></h2>

				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e('File exceeds server limit','allegra-upload-cf7'); ?></th>
						<td><input type="text" name="drag_n_drop_error_server_limit" class="regular-text" value="<?php echo esc_attr( get_option('drag_n_drop_error_server_limit') ); ?>" placeholder="<?php echo allegra_cf7_error_msg('server_limit'); ?>" /></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e('Failed to Upload','allegra-upload-cf7'); ?></th>
						<td><input type="text" name="drag_n_drop_error_failed_to_upload" class="regular-text" value="<?php echo esc_attr( get_option('drag_n_drop_error_failed_to_upload') ); ?>" placeholder="<?php echo allegra_cf7_error_msg('failed_upload'); ?>" /></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e('Files too large','allegra-upload-cf7'); ?></th>
						<td><input type="text" name="drag_n_drop_error_files_too_large" class="regular-text" value="<?php echo esc_attr( get_option('drag_n_drop_error_files_too_large') ); ?>" placeholder="<?php echo allegra_cf7_error_msg('large_file'); ?>" /></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e('Invalid file Type','allegra-upload-cf7'); ?></th>
						<td><input type="text" name="drag_n_drop_error_invalid_file" class="regular-text" value="<?php echo esc_attr( get_option('drag_n_drop_error_invalid_file') ); ?>" placeholder="<?php echo allegra_cf7_error_msg('invalid_type'); ?>" /></td>
					</tr>
				</table>
	...
	// Save admin settings
	function allegra_upload_register_settings() {
